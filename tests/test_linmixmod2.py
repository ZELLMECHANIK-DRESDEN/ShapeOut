...
def test_linmixmod():
    ...
    assert np.allclose([Result_1["p-Value (Likelihood Ratio Test)"]], [0.000602622503360039])

    #2.: Differential Deformation in a generalized linear mixed model
    Result_2 = linmixmod(xs=xs,treatment=treatment1,timeunit=timeunit1,model='glmm')
    assert np.allclose([Result_2["Estimate"]], [4.5385848573783898])
    assert 'BOOTSTAP-DISTRIBUTIONS' in Result_1['Full Summary']
    assert 'GENERALIZED' in Result_2['Full Summary']
    assert np.allclose([Result_2["p-Value (Likelihood Ratio Test)"]], [0.000556063024310929])
    ...
    Result_3 = linmixmod(xs=xs,treatment=treatment3,timeunit=timeunit3,model='lmm')
    ...
    assert np.allclose([Result_3["p-Value (Likelihood Ratio Test)"]], [0.000331343267412872])
    ...
    Result_4 = linmixmod(xs=xs,treatment=treatment4,timeunit=timeunit4,model='glmm')
    ...
    assert np.allclose([Result_4["p-Value (Likelihood Ratio Test)"]], [0.00365675950677214])
